...
function getPolynom(...rest) {
  ...
  if (rest.length === 2) {
    return (x) => rest[0] * x + rest[1];
  }

  if (rest.length === 3) {
    return (x) => rest[0] * x * x + rest[1] * x + rest[2];
  }
  return null;
}
...
function retry(func, attempts) {
  return function tryFunc() {
    let tryAtt = 0;
    for (tryAtt = 0; tryAtt < attempts; tryAtt += 1) {
      try {
        return func();
      } catch (error) {
        // tryAtt += 1;
...
function logger(func, logFunc) {
  return function f(...x) {
    ...
    const str = x.map((a) => JSON.stringify(a)).join(',');
...
function getIdGeneratorFunction(startFrom) {
  let testCount = startFrom;

  return function genId() {
    const result = testCount;
    testCount += 1;
    return result;
  };
}
